...
> 참고: 현재 활성 버전은 v14.1 - Metric Definition & Source Lineage Stabilization입니다. 이 문서는 v13에서 도입된 Tax Review Pack Generator의 이력을 설명합니다.
...
Tax Issue Matrix는 Peer 기반 Red Flag와 보유세 정합성, FFO proxy 현금유출 스트레스를 하나의 실무 검토 표로 변환합니다.
...
## 6. FFO proxy 현금유출 스트레스

보유세 증가율과 FFO proxy 스트레스 가정을 사용해 추가 현금유출과 보유세 / FFO proxy 부담을 계산합니다. Tax mode 상단의 `Tax 분석 가정` 패널에서 주요 가정을 조정할 수 있습니다.
