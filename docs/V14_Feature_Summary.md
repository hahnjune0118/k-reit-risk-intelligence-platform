...
v14.1은 v14의 Tax Workflow Control 구조를 유지하면서 핵심 지표의 명칭, 실제 계산식, 데이터 출처, 한계를 더 명확하게 설명하는 안정화 버전입니다.
...
결과 표는 공시가격 또는 장부가액, 과세표준 추정, 적용 세율, 추정 보유세, FFO proxy 대비, 영업수익 대비, Peer 대비 위치, source limitation을 함께 보여 줍니다.
...
- FFO proxy denominator 존재 여부
- 공시가격 또는 기준시가 입력값 존재 여부
- 보유세/FFO proxy 및 공시가격/장부가액 비율의 비정상 구간
...
- `tax_review_memo_{company_name}_v14_1.md`
- `tax_issue_matrix_{company_name}_v14_1.csv`
- `holding_tax_reconciliation_{company_name}_v14_1.csv`
- `tax_request_list_{company_name}_v14_1.csv`
- `tax_review_pack_{company_name}_v14_1.zip`

## 7. Non-SK 안정성

`reit_master.csv`의 모든 회사는 Tax Summary, bridge, issue matrix, reconciliation, FFO proxy stress, request list, memo, banner, source expander를 생성할 수 있어야 합니다.
