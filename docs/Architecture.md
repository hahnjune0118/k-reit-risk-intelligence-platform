...
- `data/reit_tax_snapshot.csv`: Tax Review Pack 생성을 위한 회사별 보유세 Snapshot
...
`data_availability.py`는 선택 회사별 상세 데이터 가용성을 한 곳에서 판정합니다. 반환값에는 회사 전체 재무 Snapshot, Peer Snapshot, Tax Snapshot, 자산별 보유세 proxy, 임차인 상세, 차입금 만기, Cap rate 상세의 가용성과 `source_type`, `source_note`가 포함됩니다.

회사별 상세 자산·차입금·보유세 데이터가 없는 경우에는 빈 DataFrame과 `detail_data_available = False` 상태를 전달합니다. 이때 Assurance와 Tax 화면은 SK리츠 등 다른 회사의 샘플 데이터를 재사용하지 않고, 회사 전체 Peer Benchmark/재무 Snapshot 중심의 proxy 결과를 표시합니다.

v13 Tax Review Pack 흐름:
...
1. 자산별 상세 데이터가 있으면 자산 우선순위, RMM, KAM, 체크리스트를 자산 단위로 표시
2. 자산별 상세 데이터가 없으면 회사 전체 Snapshot과 Peer Metric으로 자산·임차인 proxy, 차입금 만기·차환 proxy, 가치·NAV proxy 표를 표시
3. 자산별 Cap rate가 부족한 회사에는 N/A 중심 NAV 카드를 표시하지 않고, 회사 전체 proxy 표로 대체

Peer Benchmark 흐름은 v12에서 도입된 구조를 유지합니다. `calculations_peer.load_peer_snapshot()`으로 snapshot을 로딩하고, `calculate_peer_metrics()`와 `red_flag_engine.py`를 통해 선택 회사의 상대 위험 위치를 계산합니다.
...
공시가격 데이터는 공개 리뷰 환경에서 실시간 조회가 제한될 수 있으므로, Tax 화면은 `data/reit_tax_snapshot.csv`, 예시 데이터, 선택적 CSV 업로드를 통해 계속 사용할 수 있게 설계했습니다. 회사별 상세자료가 부족한 경우에는 `source_type = peer_snapshot_estimate`로 표시하며, 신고 목적 세액이 아닌 예비 검토용 추정값임을 UI에서 안내합니다.
