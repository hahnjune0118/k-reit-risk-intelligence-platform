...
> 참고: 현재 활성 버전은 v14.1 - Metric Definition & Source Lineage Stabilization입니다. 이 문서는 v13 데이터 품질 점검 이력을 보존합니다.
...
| `sk_reit_latest_kpis.csv` | SK리츠 KPI sample | 2 | 22 | FFO proxy, 장부NAV proxy, leverage, WALE | Peer Snapshot KPI와 기준 차이 가능 |
...
이 결과는 보유세 자체가 과대라는 결론이 아니라, 현재 sample 추정치 기준으로 FFO proxy 대비 현금유출 민감도가 크게 나타난다는 의미다.
...
5. 모든 회사에 대해 Tax Issue Matrix, Holding Tax Reconciliation, Request List, FFO proxy stress, Memo 생성 regression test를 추가한다.
```
