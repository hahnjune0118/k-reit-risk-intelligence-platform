...
모든 회사가 Peer Snapshot과 Tax Snapshot을 보유하므로 v13 Tax Review Pack의 기본 산출물은 생성 가능하다. 다만 현재 공식 자산별 tax dataset은 없고, SK리츠만 자산상세 sample을 별도로 보유한다. 따라서 모든 회사의 Tax 결과는 기본적으로 `회사 전체 추정` grain이라는 점을 화면과 memo에 계속 표시해야 한다.
...
핵심 이슈는 `official_price_growth_placeholder`다. 이름과 설명상 공시가격 상승률 rule이지만 현재 metric이 `holding_tax_to_ffo`다. v13 Tax Review Pack에서는 `official_price_growth_5y` 또는 별도 growth metric으로 분리하는 것이 안전하다.
...
현재 상태에서는 모든 회사가 주요 v13 Tax Review Pack 산출물을 생성할 수 있다. 실패 회사는 없다. 다만 산출 가능성과 데이터 신뢰도는 다르다. 대부분 회사는 asset-level 실자료가 아니라 회사 전체 Snapshot 기반이다.
