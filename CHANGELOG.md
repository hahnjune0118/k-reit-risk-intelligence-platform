...
- Tax 화면을 결론, 이슈·요청자료, 계산조서, 근거·다운로드의 네 탭으로 재구성
- 재계산액, Evidence Coverage, 실제 고지서 대사 Coverage와 P0/P1 이슈를 첫 탭에 배치
- 기존 Tax Issue Matrix와 Request List를 실행 가능한 통합 검토표로 연결
- 계산조서와 Source Lineage를 상세 단계로 유지해 추적성 보존
- 중립적 Business Process Case Brief와 Requirements Definition 추가
- Tax 계산 엔진, 세법 해석, 공식 입력자료와 불변값은 변경하지 않음
