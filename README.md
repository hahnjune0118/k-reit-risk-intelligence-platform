...
이 프로젝트는 상장 K-REIT의 Tax 업무를 자동화하는 Streamlit 기반 워크플로 앱입니다.
공시자료, Peer Snapshot, 공시가격·보유세 추정 가정, 재무지표를 연결해 다음 산출물을 생성합니다.
- Tax Issue Matrix
- Holding Tax Reconciliation
- Tax Request List
- Tax Review Memo Draft

현재 버전은 **v13 - Tax Review Pack Generator**입니다. 신고 목적의 확정 세액, 법률의견, 공식 세무자문을 제공하지 않으며 공개자료와 Snapshot 기반의 예비 검토를 돕습니다.
...
`source_type = "sample_snapshot"`인 데이터는 공개 포트폴리오 검토용 예시 데이터입니다. Tax 화면은 `source_type`과 `source_note`를 상단 banner, 공시가격 자료 출처, Tax 데이터 기준 expander에 표시해 official, snapshot, estimate 성격을 구분합니다. 감사된 자료나 공식 확정 자료로 과도하게 해석하지 않도록 UI와 문서에서 구분합니다.
