...
def render_deals_mode(latest_kpi: pd.Series, scenario: dict, market_snapshot: dict, historical_panel: pd.DataFrame, p_nav_multiple: float, p_ffo_multiple: float, required_dividend_yield_pct: float):
    ...
    st.caption("목적: Buy-side/Sell-side 관점에서 현재 시가총액이 장부NAV proxy와 FFO proxy 대비 저평가인지, 아니면 리스크를 반영한 가격인지 확인합니다.")
    ...
    if backtest is not None and not backtest.empty:
        ...
        st.caption("금리 상승기에는 NAV proxy 기반 모델이 실제 시장가치를 과대평가할 수 있습니다. 이 경우 시장은 차환위험, 요구수익률 상승, 배당 불확실성을 장부NAV proxy보다 빠르게 반영했을 수 있습니다.")
    else:
        st.info("KRX 주가·시가총액 데이터가 연결되면 실제 시장가치와 모델 추정가치의 backtesting이 표시됩니다.")

    st.success(
        "Deals 해석: Buy-side는 시나리오 후 장부NAV proxy/FFO proxy를 기준으로 가격 보정을 검토하고, Sell-side는 P/NAV proxy 할인의 원인이 되는 차환·평가·배당 리스크를 사전에 설명해야 합니다."
    )
