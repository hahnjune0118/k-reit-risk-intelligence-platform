...
with app.setup:
    ...
    _component_css = load_css()
    if not _component_css.strip():
        raise RuntimeError("검증된 Marimo CSS가 비어 있습니다.")

    def styled_html(content):
        return mo.Html(
            '<style data-k-reits-component-styles="true">'
            + _component_css
            + "</style>"
            + content
        )
...
def _(
    ...
    risk_header = styled_html(compact_header(
        title="종합 위험 및 시나리오",
    ...
    controls = mo.vstack([
        ...
        styled_html(callout("neutral", "계산 기준", "전망 입력은 확률가중 preset에 반영됩니다. 사용자 설정은 금리·스프레드·성장률·물가를 동일 시나리오 엔진의 충격값으로 변환합니다.")),
    ...
    metrics = styled_html('<section class="dense-panel"><div class="dense-panel-title"><h2>핵심 판단</h2><span>현재 → 시나리오 후</span></div>' + dense_metric_grid([
    ...
    peer_table = styled_html('<section class="dense-panel dense-table"><div class="dense-panel-title"><h2>Peer Benchmark</h2><span>8개 상장리츠 저장 시점 자료</span></div>' + html_table(risk_view.peer_comparison, numeric_formats={"현재": "{:,.2f}", "Peer 중앙값": "{:,.2f}", "백분위": "{:,.1f}%"}, max_rows=8, caption="Peer Benchmark 비교") + '</section>')
    ...
    if lower_detail_select.value == "최근 흐름":
        lower_body = mo.vstack([styled_html('<div class="decision-strip">공시된 기간별 총자산·차입금·장부 NAV proxy 흐름입니다. 인과관계가 아니라 위험 신호로만 사용합니다.</div>'), mo.ui.plotly(detail_charts.historical, config={"responsive": True, "displaylogo": False, "displayModeBar": False})], gap=0.3)
        ...
        lower_body = mo.hstack([
            mo.vstack([styled_html(f'<div class="decision-strip">{concentration_summary}</div>'), mo.ui.plotly(detail_charts.asset_concentration, config={"responsive": True, "displaylogo": False, "displayModeBar": False})], gap=0.3),
            styled_html('<div class="dense-table">' + html_table(risk_view.tenant_exposure, columns=["major_tenant", "tenant_credit", "portfolio_value_share_pct"], labels={"major_tenant": "주요 임차인", "tenant_credit": "신용도", "portfolio_value_share_pct": "평가액 비중"}, numeric_formats={"portfolio_value_share_pct": "{:,.1f}%"}, max_rows=8, caption="임차인 집중도") + '</div>'),
        ...
        lower_body = mo.vstack([
            styled_html('<div class="decision-strip">' + (f"총차입금 {total_debt/1000:,.1f}십억원 · 1년 내 만기 {near_pct:.1f}% · 차환금리 충격에 따른 추가 이자비용 {scenario['incremental_interest']:,.1f}백만원" if pd.notna(total_debt) else "회사별 차입금 만기 상세자료 미제공") + '</div>'),
...
def _(
    ...
    audit_header = styled_html(compact_header(
    ...
    scope_controls = mo.vstack([
        ...
        styled_html(callout("neutral", "감사기준서 근거", "중요왜곡표시위험의 평가와 대응은 계정·주장·감사증거 상태를 함께 고려하는 모의 감사계획 단계의 선별입니다.")),
    ...
    rmm_table = styled_html('<section class="dense-panel dense-table"><div class="dense-panel-title"><h2>RMM와 감사대응</h2><span>선택 시 상세절차 연동</span></div>' + html_table(filtered_rmm, columns=["감사영역", "관련 계정과목", "경영진 주장", "발생가능성", "영향", "감사증거 상태", "권장 감사절차"], max_rows=8, caption="중요왜곡표시위험과 감사대응") + '</section>')
    selected_rmm = risk_view.rmm[risk_view.rmm["감사영역"].eq(rmm_select.value)].iloc[0]
    rmm_summary = styled_html('<div class="mini-stat-grid">' + mini_stat("주요 중요왜곡표시위험", rmm_select.value, str(selected_rmm["RMM 신호"]), "warning") + mini_stat("현재 미해결 감사증거", f"P0 {assurance_view.kpis['p0_open']} · P1 {assurance_view.kpis['p1_open']}", "고지서·과세구분·소유관계 추가 검증", "critical") + '</div>')

    if audit_detail_select.value == "감사계획·위험평가":
        audit_detail = styled_html(panel("선택 위험의 감사계획", f'<p><strong>관련 계정:</strong> {selected_rmm["관련 계정과목"]}</p><p><strong>경영진 주장:</strong> {selected_rmm["경영진 주장"]}</p><p><strong>위험 판단:</strong> {selected_rmm["왜 중요한가"]}</p><p><strong>대응 감사절차:</strong> {selected_rmm["권장 감사절차"]}</p>'))
        ...
        audit_detail = styled_html('<div class="dense-table">' + html_table(priority_assets, columns=["자산", "평가액비중_%", "Cap_rate_%", "시나리오가치변화_%", "감사중점점수", "감사 우선순위", "중점검토사유"], numeric_formats={"평가액비중_%": "{:,.1f}%", "Cap_rate_%": "{:,.2f}%", "시나리오가치변화_%": "{:,.1f}%", "감사중점점수": "{:,.0f}"}, max_rows=8, caption="자산별 감사 우선순위") + '</div>')
    elif audit_detail_select.value == "통제·실증절차":
        audit_detail = styled_html(panel("통제테스트·실증절차·요청자료", '<p><strong>IPE 검증:</strong> 입력자료 모집단 완전성, 계산식 접근권한, 검토 증적 확인</p><p><strong>외부자료 검증:</strong> 평가보고서·차입약정·등기·고지서 원문 대사</p><p><strong>통제테스트:</strong> 변경 승인, 독립 검토, 예외 후속조치 표본검사</p><p><strong>실증절차:</strong> 재계산·외부조회·후속입금·기간귀속 테스트</p><p><strong>요청자료:</strong> 외부평가 입력, 임대차계약, 차입금 master, 보유세 과세내역서</p>'))
    else:
        audit_detail = styled_html('<div class="dense-table">' + html_table(risk_view.kam_candidates, max_rows=6, caption="KAM 후보 모의 검토") + html_table(risk_view.icfr_controls, max_rows=8, caption="내부회계관리제도 핵심통제") + '</div>')
    ...
    tax_metrics = styled_html('<div class="mini-stat-grid">' + mini_stat("공식자료 재계산액", format_eok(assurance_view.base_total), "원 단위 " + format_krw(assurance_view.base_total, 5), "ok") + mini_stat("실제 고지세액", "미확인", "2026 고지서 미수령", "critical") + mini_stat("고지서 대사율", str(assurance_view.kpis["notice_coverage"]), "미대사", "critical") + mini_stat("미해결 사항", f"P0 {assurance_view.kpis['p0_open']} · P1 {assurance_view.kpis['p1_open']}", "추가 감사증거 필요", "warning") + '</div>')
    calc_table = styled_html('<div class="dense-table">' + html_table(calc, columns=["tax_name", "tax_base", "tax_rate", "calculated_tax", "calculation_status"], labels={"tax_name": "세목", "tax_base": "과세표준", "tax_rate": "세율", "calculated_tax": "산출세액", "calculation_status": "근거 상태"}, numeric_formats={"tax_base": "{:,.0f}", "tax_rate": "{:,.6f}", "calculated_tax": "{:,.0f}"}, status_columns=["calculation_status"], max_rows=9, caption="세목별 계산조서") + '</div>')
    tax_column = mo.vstack([
        ...
        calc_table,
        mo.accordion({"산식·법적 근거·원 단위 값": styled_html(callout("neutral", "근거 미확보 시 계산 차단(Fail-closed)", "공식 근거가 없는 값은 장부가액, Peer 비율 또는 0으로 대체하지 않습니다. 상세 산식과 조문은 세목별 검토조서와 내보내기 파일에 보존됩니다."))}),
...
def _(
    ...
    conclusion_header = styled_html(compact_header(
    ...
    evidence_stats = styled_html('<div class="mini-stat-grid">' + mini_stat("핵심 입력자료 근거 확보율", str(assurance_view.kpis["evidence_coverage"]), "주소·PNU·지가·건물가액·납세의무자", "ok") + mini_stat("IPE 완전성·정확성", "5/5 확인", "계산 입력과 원천자료 연결", "ok") + mini_stat("외부자료 신뢰성", "공식 원문·해시", "검증상태와 계산 사용 여부 기록", "ok") + mini_stat("고지서 확보", "미확인", "실제 고지세액·과세코드 미입수", "critical") + '</div>')
    evidence_table = styled_html('<div class="dense-table">' + html_table(assurance_view.evidence_matrix, columns=["metric_or_fact", "value", "source_name", "verification_status", "used_in_calculation"], labels={"metric_or_fact": "지표·사실", "source_name": "원천자료", "verification_status": "검증 상태", "used_in_calculation": "계산 사용"}, status_columns=["verification_status"], max_rows=10, caption="감사증거 매트릭스") + '</div>')
    lineage_table = styled_html('<div class="dense-table">' + html_table(assurance_view.source_lineage, max_rows=8, caption="원천자료 계보") + '</div>')
    ...
    flow = styled_html(reconciliation_flow(format_krw(assurance_view.base_total, 5)))
    ...
    issue_table = styled_html('<section class="dense-panel dense-table"><div class="dense-panel-title"><h2>P0/P1 미해결 사항</h2><span>증거·담당절차·다음 조치 연결</span></div>' + html_table(visible_issues, columns=["priority", "tax_issue", "evidence_status", "potential_tax_effect", "required_document", "responsible_reviewer", "resolution_status"], labels={"priority": "우선순위", "tax_issue": "미해결 사항", "evidence_status": "증거 상태", "potential_tax_effect": "예상 영향", "required_document": "필요 감사증거", "responsible_reviewer": "담당 절차", "resolution_status": "상태"}, status_columns=["evidence_status", "resolution_status"], max_rows=8, caption="P0 P1 미해결 사항") + '</section>')
    reconciliation = mo.vstack([
        flow,
        styled_html(callout("critical", "대사 결론 보류", "실제 고지서가 확보되지 않아 실제 고지세액과 차이는 계산하지 않았으며, 대사 결론도 보류했습니다.")),
    ], gap=0.45).style({"min-width": "0", "overflow": "hidden"})

    conclusion = styled_html('<section class="dense-panel"><h2>검토자 결론</h2><div class="mini-stat-grid">' + mini_stat("현재 판단", "추가 감사증거 필요", "독립적 재계산 완료·고지서 대사 미완료", "warning") + mini_stat("결론 승인 상태", "보류", "모의 감사검토 단계", "critical") + '</div>' + panel("판단 근거", f'<p>공식 입력자료 기반 재수행액은 <strong>{format_eok(assurance_view.base_total)}</strong>입니다. 실제 고지서·과세구분 코드·과세기준일 등기 및 신탁상태가 미확인되어 확정세액으로 결론내리지 않습니다.</p>') + panel("미해결 감사증거 공백", bullet_list(assurance_view.snapshot_payload.get("open_items", []))) + '</section>')
    request_table = styled_html('<div class="dense-table">' + html_table(assurance_view.request_list, columns=["priority", "required_document", "request_reason", "reviewer_status"], labels={"priority": "우선순위", "required_document": "추가 요청자료", "request_reason": "후속조치", "reviewer_status": "상태"}, status_columns=["reviewer_status"], max_rows=8, caption="추가 요청자료") + '</div>')
    exports = mo.vstack([
        mo.md("### 검토조서 내려받기"),
        ...
        styled_html(callout("neutral", "실행환경", "로컬 Python과 Molab Python notebook에서는 파일 생성을 지원합니다. 브라우저 전용 WASM에서는 Excel 생성 라이브러리와 파일 내려받기가 제한될 수 있습니다.")),
