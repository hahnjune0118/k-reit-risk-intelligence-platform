...
from calculations_tax_review_pack import (
    ...
    build_kpi_source_badges,
    build_operating_cash_flow_stress,
    build_source_detail,
...
from data_source_policy import get_source_policy, source_badge_label
...
from ui_peer import build_peer_metric_table, style_risk_review_table
...
def _render_tax_assumption_panel(assumptions: dict, scenario: dict) -> dict:
    with st.expander("Tax 분석 가정", expanded=True):
        ...
        with c5:
            default_ocf_stress = float(scenario.get("ffo_haircut_pct", DEFAULT_TAX_ASSUMPTIONS_V14["ffo_stress_pct"]) or DEFAULT_TAX_ASSUMPTIONS_V14["ffo_stress_pct"])
            operating_cash_flow_stress_pct = st.slider(
                "영업활동현금흐름 감소 스트레스",
                min_value=0.0,
                max_value=30.0,
                value=float(assumptions.get("operating_cash_flow_stress_assumption", assumptions.get("ffo_stress_assumption", default_ocf_stress))),
                step=1.0,
                key="v14_operating_cash_flow_stress_assumption",
            )
        st.caption(
    ...
    return {
        ...
        "operating_cash_flow_stress_assumption": operating_cash_flow_stress_pct,
        "operating_cash_flow_stress_pct": operating_cash_flow_stress_pct,
        "ffo_stress_assumption": operating_cash_flow_stress_pct,
        "ffo_stress_pct": operating_cash_flow_stress_pct,
...
def _render_tax_source_scope_banner(target_company: str, company_profile: dict, source_summary: dict):
    ...
    with st.container(border=True):
        ...
        st.caption(
            ...
            f"근거: {source_summary.get('source_badge', source_badge_label(source_summary.get('source_type')))} "
            f"{source_summary.get('korean_label', policy.korean_label)} / "
            f"증거 신뢰도: {source_summary.get('evidence_confidence', 'insufficient')}"
        )
        st.caption(f"근거 메모: {source_summary['source_note']}")
...
def _render_peer_tax_section(peer_context: dict | None):
    ...
    with st.expander("Peer 기반 보유세 부담 비교", expanded=False):
        metric_table = build_peer_metric_table(
            ...
            target_company,
            {
                "holding_tax_to_operating_cash_flow": "추정 보유세/영업활동현금흐름 proxy",
                "holding_tax_to_operating_revenue": "추정 보유세/영업수익",
                "official_price_to_investment_property": "기준가격 Snapshot/proxy/투자부동산",
                "dividend_to_operating_cash_flow": "배당/영업활동현금흐름 proxy",
        ...
        st.dataframe(
            pd.DataFrame(
                [
                    ...
                    {"항목": "영업활동현금흐름 분모", "값": "있음" if validation.get("operating_cash_flow_proxy_exists") else "부족"},
                    {"항목": "별도 FFO proxy", "값": "산출" if validation.get("ffo_exists") else "조정항목 부족"},
                    {"항목": "기준가격 입력값", "값": "있음" if validation.get("price_basis_exists") else "부족"},
                    {"항목": "공식 기준가격 검증", "값": "검증" if validation.get("official_price_verified") else "미검증"},
                    ...
                    {"항목": "법적 소유자 확인", "값": "확인" if validation.get("legal_owner_confirmed") else "미확인"},
                    {"항목": "세목 범위 확인", "값": "확인" if validation.get("tax_components_complete") else "미확인"},
                    {"항목": "법령 검증", "값": "완료" if validation.get("legal_verification_complete") else "법률 검토 필요"},
                ]
            ),
            width="stretch",
            hide_index=True,
...
def render_tax_mode(
    ...
    st.caption(
        "기준가격 Snapshot/proxy, 장부가액, Peer 자료, 영업활동현금흐름 proxy를 연결해 회사 전체 보유세 부담과 요청자료 우선순위를 예비 검토합니다."
    )
    ...
    operating_cash_flow_stress = build_operating_cash_flow_stress(
        latest_kpi,
        annual_summary,
        tax_pack_assumptions["holding_tax_increase_pct"],
        tax_pack_assumptions["operating_cash_flow_stress_pct"],
    )
    tax_source_status = get_tax_source_status(target_company, company_tax_data)
    issue_matrix = build_tax_issue_matrix(
        tax_flags,
        reconciliation,
        operating_cash_flow_stress,
    ...
    peer_metrics = (peer_context or {}).get("peer_metrics", pd.DataFrame())
    selected_peer_row = get_company_peer_profile(peer_metrics, target_company)
    kpi_source_badges = build_kpi_source_badges(selected_peer_row, tax_bridge, source_summary)
    ...
    tax_badge = source_summary.get("source_badge", source_badge_label(source_summary.get("source_type")))
    taxpayer_note = "납세의무자 확인" if validation.get("taxpayer_confirmed") else "납세의무자 미확인"
    m1.metric(f"{latest_year} 회사 전체 보유세 부담 추정", format_bn_krw(latest_total_tax))
    m1.caption(f"{tax_badge} · {taxpayer_note} · 신고 목적 확정 세액 아님")
    m2.metric("확인된 과거 추이", format_pct_from_100(cumulative_increase) if has_actual_history else "데이터 부족")
    m2.caption("[데이터 부족]" if not has_actual_history else tax_badge)
    m3.metric("회사 전체 과세표준 proxy", format_bn_krw(latest_tax_base))
    m3.caption(f"{tax_badge} · Snapshot 기반")
    m4.metric("증거 신뢰도", source_summary.get("evidence_confidence", "insufficient"))
    m4.caption(source_summary.get("korean_label", "데이터 부족"))

    with st.expander("핵심 KPI별 근거 배지", expanded=False):
        st.caption("각 지표의 값, 계산방법, 근거 유형과 제한사항을 개별적으로 표시합니다.")
        st.dataframe(kpi_source_badges, width="stretch", hide_index=True, height=360)
    ...
    with c2:
        ...
        fig_asset_tax = px.bar(
            ...
            y="asset_name",
            orientation="h",
            title=f"{latest_year} 회사 전체/자산별 보유세 부담 추정",
        )
    ...
    st.markdown("## 회사 전체 보유세 부담 추정 브리지")
    st.caption("근거 유형, 증거 신뢰도, 계산 모델, 세목 범위와 납세의무자 확인 상태를 함께 표시합니다. 회사 전체 추정 행은 자산별 고지세액 대사 전까지 예비 분석입니다.")
    ...
    st.caption("정량 위험도, 증거 신뢰도와 검토 상태를 분리하여 실무 검토 항목으로 변환합니다.")
    st.dataframe(style_risk_review_table(issue_matrix), width="stretch", hide_index=True, height=310)

    st.markdown("## 보유세 정합성 검토")
    st.caption("투자부동산 장부가액, 기준가격 Snapshot/proxy, 과세표준 proxy, 회사 전체 보유세 부담 추정을 연결한 예비 대사표입니다.")
    st.dataframe(reconciliation, width="stretch", hide_index=True, height=240)

    st.markdown("## 영업활동현금흐름 감소 스트레스")
    if operating_cash_flow_stress.empty:
        st.warning("영업활동현금흐름 감소 시나리오를 계산할 수 없습니다. KPI와 보유세 요약 데이터를 확인하세요.")
    else:
        c1, c2 = st.columns([1.05, 0.95])
        with c1:
            st.dataframe(operating_cash_flow_stress, width="stretch", hide_index=True, height=240)
        with c2:
            fig_cash = px.bar(operating_cash_flow_stress, x="항목", y="금액(억원)", title="보유세 부담 추정 스트레스")
    ...
    with st.expander("Tax 데이터 기준 및 source detail", expanded=False):
        st.caption(f"근거 유형: {source_summary.get('source_badge')} / 근거 메모: {source_summary['source_note']}")
    ...
    st.warning(
        "본 검토는 공개된 연결 또는 별도재무정보와 회사 전체 Snapshot을 기초로 한 제한적인 screening estimate입니다. "
        "개별 자산의 법적 소유자, 납세의무자, 토지·건축물별 과세표준, 합산·분리과세, 감면, 신탁·SPC·펀드 구조 및 실제 고지세액을 확인하기 전에는 "
        "신고 목적 세액이나 법률적 결론으로 사용할 수 없습니다."
    )
